...
M1 第一批已可使用：可以创建和编辑随手记，上传 PDF、DOCX、Markdown、TXT，浏览资料列表并下载原文件。URL 抓取、正文解析、AI 分类、搜索和问答仍在开发中。

## 现在可以做什么

- 创建、查看和编辑随手记，每次实质修改保留新版本
- 上传 PDF、DOCX、Markdown、TXT，并限制文件类型和大小
- 流式保存原文件并计算 SHA-256，相同内容复用一份 Blob
- 浏览全部资料及“待处理”状态
- 下载已保存的原文件

下一步是 URL 入库与基础文档解析。AI 分类、混合检索和带引用问答按[开发路线图](docs/ROADMAP.md)逐步加入；邮件转入、浏览器插件、法律/合同深度解析和知识图谱安排在个人 Beta 之后。

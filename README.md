...
- **Excel 采用“数据集或待治理”的产品边界**：保留标准二维表的 Parquet/DuckDB
  精确查询；第二行表头、标题行、常见合并表头等通用形态可以逐步增加受校验的映射。
  无法可靠验证的复杂结构保留原文件和诊断，提示用户先治理，不再降维成长文本向量。
...
- 连接 PostgreSQL 或 MySQL，只读浏览 Schema/表并导入可重复更新的本地数据快照；
  可选默认的过期后台刷新、严格新鲜或仅手动刷新，问答会明确展示快照时间。
...
- 文档详情可分页预览搜索和问答实际使用的在线切片，并显示基础结构质量诊断；读取
  不调用模型、不修改索引。高级诊断可在有限窗口内让 AI 建议原文块边界，程序校验
  完整性并对比样例，但候选不自动替换在线索引。
- PostgreSQL 全文检索与 pgvector 向量检索通过 RRF 融合。
- 二维数据集生成可重建的 Parquet 版本，由 DuckDB 下推筛选、投影、排序、分组与
  聚合；API/MCP 只接受受控查询计划，不开放任意 SQL。
- 数据集字段保留类型、空值、唯一值、范围与样例等程序画像；配置对话模型后
  可批量生成受校验的字段说明、单位与别名。AI 语义只用于界面理解和问法映射，
  不修改字段、类型、Parquet 或原始行；数据库快照刷新默认复用未变化字段的说明，
  只补新增、类型变化或缺失字段，也可显式全量重建；模型不可用时仍可精确查询。
- XLSX 解析按实际值/公式范围排除纯样式空白，保留资源保护。只有通过结构校验的区域
  进入数据集目录和精确查询；非规则区域保留坐标、合并关系和风险诊断，页面标记为
  待治理，不生成全文切片或向量。AI 以后只能建议常见结构映射，由程序校验后激活。

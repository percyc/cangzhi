...
### 2026-09-21 数据库来源范围与表管理优化（CZ-UX / 基线 G）

- 背景：数据库来源只显示快照总数，目录中看不出哪些表已经导入；“一键导入全部”也未
  在按钮和确认流程中充分说明数据库边界。代码复核发现 PostgreSQL 始终连接配置库，
  但 MySQL Schema 列表会枚举当前账号可见的所有非系统库，确有跨越配置库的风险。
- 变更：MySQL Schema 查询改为服务端参数化硬过滤 `database_name`，目录校验和数据读取
  复用同一范围；PostgreSQL 保持“指定数据库内的非系统 Schema”。目录接口返回已导入
  快照状态，新增连接器级已导入表清单；页面可按表名和导入状态筛选、勾选当前结果、
  批量导入所选表，并查看行数、更新时间及对应资料。全库导入降为次级操作，按钮和确认
  窗口明确显示目标数据库、Schema 数和表数。已有快照时禁止直接改成另一数据库，避免
  旧快照与新库同名表混写；可新建连接器并按需保留旧资料。
- 理由：默认路径应是先观察、再选择、后导入；保留全库导入用于明确需要的小型数据库，
  但不能把数据库账号权限误当导入范围。范围约束必须在服务端执行，不能只靠文案。
- 测试：数据库来源与工作空间专项 36 项通过，其中新增 MySQL 配置库参数校验、目录导入
  状态、快照清单及换库保护；Web ESLint、TypeScript 类型检查和 Next.js 生产构建通过。
- 部署：待本条代码提交后仅重建 API/Web；无迁移，不重启 Worker/PostgreSQL，不自动
  导入、重建或清理任何资料。
- 后续：首版仍是页面驱动的串行批量请求，超大目录的后台任务化、增量同步和远端表删除
  提醒继续留在基线 G 后续能力，不在本轮伪装成已完成。

### 2026-09-21 数据库来源管理优化部署

- `869c042` 已快进本地 `main`。API/Web旧镜像分别保留为
  `rollback-before-db-source-ux-869c042`，随后仅重建并替换 API/Web；Worker、
  PostgreSQL 未重启，没有迁移、表导入、资料重建或数据清理。
- API/Web/Worker/PostgreSQL 最终均 healthy，readiness、Web health 与设置来源页面
  返回 200，schema 保持 0033；新增快照清单接口未登录请求返回 401。运行镜像与本轮
  新构建镜像一致，启动日志无异常。
- 当前自动化浏览器没有暴露可控制页面，故未伪称完成登录态点击；维护者刷新“设置 →
  外部来源 → 数据库来源”即可看到新表管理台。代码和接口测试已覆盖导入状态与范围，
  但未用真实外部 MySQL 凭据执行导入，以免把验收变成未授权的数据操作。

### 2026-09-21 向量失败诊断与版本清理（CZ-Q04 / 基线 C）

- 背景：向量索引页面只显示失败任务数量和“重试失败任务”，没有呈现任务保存的错误；
  非生效版本只要状态仍是 `building` 就完全禁止删除，即使没有任务正在执行，用户无法
  判断应修配置、重试还是放弃版本。
- 变更：状态接口按向量版本汇总最多五类失败原因及数量，同时返回排队/重试和执行中
  任务数；页面直接展示错误、处理建议和任务状态。非生效版本在没有 `processing` 任务
  时可“停止并删除版本”，删除会在同一事务内锁定并清理排队、重试、失败任务及该版本
  向量；如仍有任务正在执行则拒绝删除并明确显示需等待的数量。生效索引仍绝不允许删除。
- 理由：重试不是所有失败的正确动作；地址、密钥、模型、维度错误必须先可见。删除构建
  候选不应受排队任务阻塞，但不能让正在运行的 Worker 对已删除版本回写，因此以行锁和
  `processing` 状态作为安全边界。
- 测试：向量生命周期与构建服务专项 56 项通过，新增失败原因聚合、构建中版本取消清理、
  执行中任务删除拒绝；Web ESLint、TypeScript 类型检查和 Next.js 生产构建通过。
...
### 2026-09-21 快速问答数据集体验（CZ-UX / 基线 C、G）

- 背景：快速问答已有“AI生成结构映射、程序校验、DuckDB/Parquet执行”的精确查询路径，
  但触发词偏统计型，趋势、对比和明细等自然问法容易退回普通检索；已有文本命中时也会
  阻止范围内数据集目录补充，页面在回答前后都不够直观。
- 变更：结构化意图增加比较、差异、变化、趋势、增长、下降、同比、环比、分布、明细和
  记录等通用表达；快速问答把语义排序文档与当前知识范围内的数据集目录候选去重合并，
  连接器和显式资料范围仍在服务端先做交集。查询结果增加数据集名称，并把已校验的聚合、
  明细/分组贡献行和警告返回页面。提问区显示可查询数据区域数量及“复杂跨表改用深度
  分析”入口；回答下直接展示可折叠结果表、命中行数、指标和资料链接。
- 理由：快速问答应覆盖单数据集的一次筛选、明细、统计和排名；跨表、多步骤推导继续由
  深度分析的 `list_datasets/get_dataset_schema/query_dataset` 工具链处理。没有开放自由
  SQL，也没有把整表向量化或让模型计算事实。
- 测试：结构化表格、快速问答和数据集API专项 66 项通过，新增趋势/比较/明细意图、候选
  合并和轻量数据集摘要；Web ESLint、TypeScript 类型检查及生产构建通过。
- 部署：待本条代码提交后更新 API/Web；无迁移，不重建数据集、切片或向量。当前运行库
  只读核对有5个数据区域、来自5份资料，5个列式产物均 ready。
- 局限：快速问答仍只执行一个数据集的一份受控计划，最多返回50行；跨表关联、复杂时间
  窗口和连续多步推导不伪装成已支持，页面明确引导深度分析。

### 2026-09-21 快速问答数据集体验部署

- `0cede41` 已快进本地 `main`。API/Web旧镜像分别保留为
  `rollback-before-quick-dataset-qa-0cede41`，仅重建替换 API/Web；Worker、PostgreSQL
  未重启，无迁移、数据集重建、切片或向量任务。
- API/Web/Worker/PostgreSQL 均 healthy，schema 0033，readiness、Web health 与问答页面
  返回 200；轻量数据集摘要接口未登录返回 401。运行 Web 产物可核验“精确查询结果”已
  进入问答页面，部署后5个数据区域、5份资料和5个 ready 列式产物数量保持不变。
- 当前自动化浏览器没有暴露可控制登录页面，未伪称完成真实模型点击问答；真实结果质量
  仍取决于问题能否映射到已有字段，复杂跨表问题应使用深度分析。

### 2026-09-22 数据集字段语义画像（CZ-UX / 基线 C、G）

- 背景：现有 `DatasetField` 已有类型、简单语义角色、空值、唯一值、样例和数值范围，
  但没有字段级说明；缩写、隐含单位和业务别名只能由列名与少量样例猜测，会影响数据集
  发现和自然语言到查询计划的映射。
- 变更：迁移0034为字段增加可选说明、单位、别名、来源和置信度。模型按数据集上下文分批生成，
  服务只接受精确匹配真实字段名的有界输出，最多自动处理320列。新数据集在 Parquet 就绪后
  低优先级尝试；历史数据集在“字段画像”页提供主动生成/重新生成。模型不可用或失败时任务
  可降级完成，不阻断导入、列式产物和精确查询。
- 使用：数据集页显示说明、单位、别名、置信度及“程序事实 / AI 建议”边界。REST v1 / MCP
  Schema 返回相同语义字段，快速问答规划器可用于问法匹配；执行器仍只允许原始字段名和白名单计划。
- 验证：数据集语义、内部接口、结构化查询、DuckDB 执行、数据库来源及 Worker 调度专项
  `140 passed`；Web ESLint、TypeScript 与 Next.js 生产构建通过。Alembic 只有一个0034 head；全链
...
### 2026-09-22 数据库快照新鲜度（CZ-UX / 基线 G）

- 背景：PostgreSQL/MySQL 导入的数据集是可复现的本地快照，但此前只有用户主动重新
  导入才会更新；REST/MCP/问答也没有返回快照时间，长期运行时容易把旧数据误解为实时。
- 变更：数据库来源增加默认“过期后台刷新”、严格新鲜和仅手动三种策略及5分钟到30天
  间隔。数据集目录、Schema、内部详情和受控查询统一返回 `source_freshness`；后台模式
  先返回绑定当前文档/Parquet版本的结果与警告，再创建延迟任务，严格模式返回
  `dataset_stale` 并立即排队，手动模式只提示。数据集目录采用两次批量查询，没有引入
  随数据集数量增长的 N+1 状态读取。
- 可靠性：刷新任务以快照ID和时间生成幂等键，支持并发去重、Worker退避重试和状态展示；
  新数据版本提交后才替换旧版本。远端读取、产物构建或提交失败时保留旧数据集和Parquet，
  不让模型执行任意远端SQL，不在同一回答中混用两个数据版本。设计见 ADR-028。
- 界面：数据库来源创建/编辑页可直接选择策略和间隔，来源卡片显示最近同步；文档数据集
  工作区显示快照时间、是否过期、是否已安排刷新。默认选项无需新手额外配置。
- 验证：新鲜度服务、后台/严格执行、接口、数据库来源、Worker成功/失败重试及优先调度
  专项 `144 passed`；Worker完整 `215 passed`；Web ESLint、TypeScript和生产构建通过；
  Alembic单一head为0035。API完整回归 `1258 passed, 2 skipped`，其余8项只因本地
  开发虚拟环境未安装既有OCR测试依赖Pillow而失败，与本次变更无关。
- 局限：仍是10万行上限内的整表/视图快照，不是CDC或增量同步；Worker停止时任务只会
...
### 2026-09-22 数据库字段语义智能增量（CZ-UX / 基线 G）

- 背景：数据库表每次重导都会创建新数据集，原流程因此为全部字段重新创建语义任务；即使
  结构完全没变，也可能重复消耗对话模型。结构检查本身是确定性操作，不应与AI重生成绑定。
- 默认行为：数据、字段顺序、推断类型、样例、空值和唯一值仍每次完整刷新。字段名和推断
  类型同时未变化且已有说明时，在新数据集版本内复制说明、单位、别名、来源和置信度；
  只有新增、类型变化或缺少说明的字段进入AI任务。全部可复用时不创建语义任务、调用数为0。
- 可选行为：数据库来源可选择 `smart`（默认）或 `full`；全量模式仍先复用旧说明作为模型
  不可用时的安全降级，但会请求重新理解全部字段。文档字段画像页也提供“智能补充 / 全量
  重新生成”，调用前显示预计字段数；单次最多320字段。
- 可观察性：导入响应和数据集profile记录策略、复用字段数、预计AI字段数；单表导入完成
  提示直接展示成本范围，后台刷新任务结果也保存相同统计。MCP/API读取到的是当前版本最终
  字段语义，不需要新增一套探索协议。
- 验证：数据库首次导入/重导、智能复用、全量重建、无待处理零调用、定向语义生成、Worker
  消费、数据集接口与知识v1专项 `64 passed`；Worker完整 `216 passed`；Web ESLint、
  TypeScript与Next.js生产构建通过；Alembic单一head为0036。
- 局限：当前复用签名是精确字段名+程序推断类型，不会因普通行值变化反复调用模型；若业务
  口径变了但字段名和类型不变，用户应选择全量重建。数据库原生表/列注释仍未接入权威说明层。
- 部署：待提交后执行0036迁移并更新API/Web/Worker；迁移只为既有来源增加默认 `smart`，
  不重导表、不自动调用模型。

### 2026-09-22 数据库字段语义智能增量部署

- `4cf3094` 已快进本地 `main`。发布前创建并校验联合备份
  `backups/cangzhi-20260922-045908`，随后重建API、Web、Worker并执行0036迁移；
  PostgreSQL容器未替换。
- 发布后API、Web、Worker、PostgreSQL均healthy，schema=0036，readiness、Web health、
  来源设置页和文档20返回200；启动日志未发现异常。发布镜像补跑字段语义与Worker智能
  选择专项6项全部通过。
- 两个既有数据库来源均采用默认 `smart`；迁移后10分钟内新增字段语义任务为0，确认发布
  没有重导远端表或调用模型。用户刷新设置页即可看到“智能增量 / 全量重新生成”选项，
  文档字段画像页可选择范围并在执行前看到预计字段数。

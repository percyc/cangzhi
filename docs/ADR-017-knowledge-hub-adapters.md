...
`search` 和 `ask` 保持无状态，适合 CLI、自动化和外部智能体。持久对话使用独立
的 `conversations`、`conversation_messages`，不会让 `/ask` 隐式产生状态。

每轮对话都根据当前问题和固定的 `KnowledgeScope` 重新检索。历史消息用于消解
上下文，不复用旧证据代替新检索；消息保存实际引用的 document/version/chunk，
保证之后仍能解释答案来源。
...
交付会话/消息数据模型和聊天式 `/ask`。验证每轮重新检索、范围固定、引用可回溯、
删除/恢复行为与知识生命周期一致。

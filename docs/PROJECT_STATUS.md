...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| Excel 有效范围与不规则结构 | `CZ-Q04` / `CZ-N01` | 首批代码与测试完成，未部署 | Codex / OpenCode；`codex/CZ-Q04-spreadsheet-sparse`。完整1445 passed/2 skipped，最终专项122 passed。XLSX 实际范围排除纯样式空白；复杂区域保留行列并回退全文，不冒充可计算明细。真实997只读解析成功（41265有效单元格）；原文候选1865个子片段，未创建生产向量。高级表头/区域理解仍待完成，不重跑历史队列 |

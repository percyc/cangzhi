...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 切片候选与统计性能发布 | `CZ-N04` / `CZ-Q03` | 发布验证中 | Codex，维护者本轮明确授权合并部署；代码 249a799 / ae32dc0，先备份与隔离镜像回归，仅更新 API/Web/Worker；不自动重建历史索引 |
| AI 辅助切片 | `CZ-N02` / `CZ-N04` | 候选首批完成，未启用 | Codex / OpenCode，codex/CZ-N04-assisted-chunking；PDF 误标题与统计性能已修，提供限额候选比较；产物版本、原子激活、旧引用保留与真实黄金集尚未完成；未部署、未重建历史索引 |
...
- 2026-09-17 PDF 误标题、向量进度 SQL 聚合与受约束 AI 候选评估开发完成，待维护者
  合并发布；不等同于全库自动 AI 切片。规则候选对照两份实际 PDF 明显减少短片段，
  尚无真实检索质量收益结论；验证结果与实现边界见 DEVLOG 与 ADR-023。

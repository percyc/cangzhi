...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 新上传优先的维护调度 | `CZ-Q05` | 已测试，待维护者启用 | Codex / OpenCode；`codex/CZ-Q05-new-upload-priority`。隔离 API/Worker 回归 850 项通过；按版本创建时间划分新资料，前台与授权历史任务按 4:1 调度。普通 Worker 仍停止，新入口尚未启动；不启动全库重建 |
...
| 历史 Word 重处理 | `CZ-Q06` | 后台队列进行中 | 2026-09-15 维护者明确要求启动；4 空间 157 份资料。公平调度上线验收时解析由 2 增至 7 份、150 份等待，新增 5 份均使用新解析器；切片、理解与向量也有完成记录，非全库完成；详见 DEVLOG |
...
- **维护暂停（2026-09-18）**：维护者要求失败和未解析旧流程全部停止，Worker 已收到
  ...
  processing，不重置失败解析。新上传后台暂缓，恢复普通 Worker 会继续旧队列，
  因此新流程只允许定向维护执行。备份 `backups/cangzhi-20260918-082746` 已校验。

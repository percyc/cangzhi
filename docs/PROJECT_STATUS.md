...
| 可选 AI 知识增强 | `CZ-N01`–`CZ-N06` | 底层首批已发布，完整功能开发中 | `8bdba8e` 已合并 main 并推送 Gitea；后续持久化/管理UI、公共增强读取、连续窗口分层概览已实现，尚未发布。语义章节、实体消歧、单元格探索、切片产物原子激活仍待接入；基础摘要分类标签保留，未启用历史处理 |

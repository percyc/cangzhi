...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 可选 AI 知识增强 | `CZ-N01`–`CZ-N06` | 底层首批已发布，完整功能开发中 | Codex / OpenCode；`8bdba8e` 已合并 main 并推送 Gitea。ADR-024、策略/地图纯服务纳入发布镜像，完整回归 989 项通过。持久化、任务、UI 与探索入口尚未接入；基础摘要分类标签保留，未启用历史处理 |

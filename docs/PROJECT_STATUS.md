...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 文档内证据选择评测 | `CZ-R01` / `CZ-R02` | 开发与候选验证完成，待维护者合并部署 | Codex / OpenCode；codex/CZ-R02-evidence-selection。保留主证据＋至多1条补充证据，本地30题来源短语覆盖21→29，无已通过题退步；不是语义准确率或全库黄金集。搜索页/REST/MCP/深度分析共用；不重切、不恢复旧队列，正式服务未更新 |

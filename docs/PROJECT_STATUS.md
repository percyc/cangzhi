...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 通用碎片治理 | `CZ-N04` / `CZ-R04` | 原文定位与碎片防退化代码完成，待真实向量对照 | Codex；codex/CZ-N04-structural-fragments。9份只读候选定位可核验，其中2份相对当前切片的碎片指标退步，触发新拒绝条件；跨格式关键词/上下文集成通过。未部署、未改正式索引或恢复旧队列 |

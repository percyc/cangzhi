...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| 全页面验收与发布 | `CZ-Q08` | 进行中 | 负责人 Codex；分支 `codex/CZ-Q08-release-review`；桌面/手机页面检查、构建验证、备份、推送 origin/main 与更新服务；本次部署由维护者在会话中明确授权 |
...
- 2026-09-14 解析可靠性第一批完成验证：派生文本 Unicode 规范化、元数据字段
  ...
  按维护者要求合并至本地 main；未推送、未部署、未重处理历史资料。
...
- 优先完成解析可靠性后续：Worker 心跳与超时任务恢复、DOCX 段落表格原序保留；
  第一批代码由维护者部署后，再评估历史失败资料的定向重试。

...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| Worker 分阶段公平调度 | `CZ-Q05` | 已验证，待维护者合并/部署 | 负责人 Codex / OpenCode；分支 codex/CZ-Q05-stage-scheduling，代码 7f59ced。分阶段轮转、每次领取一条、每轮最多尝试 10 条；隔离 API/Worker 700 项通过，真实 PostgreSQL 跳锁验证通过。心跳/租约恢复仍待完成；生产队列本轮未修改 |
| 历史 Word 重处理 | `CZ-Q06` | 后台队列进行中 | 2026-09-15 维护者明确要求启动；Codex 经校验备份后排入 4 空间 157 份资料，2 份 WebDAV 原文件读取成功。初次复核 2 份解析完成、155 份等待；文档 428 的解析与切片已验证。向量仍受既有队列积压影响，非全部完成；详见 DEVLOG |

...
| 主题 | 关联 ID | 状态 | 备注 |
|---|---|---|---|
| Excel 有效范围与不规则结构 | `CZ-Q04` / `CZ-N01` | 首批已授权部署，997向量后台生成 | 分支 `codex/CZ-Q04-spreadsheet-sparse` / `4218fd3`（尚未并main/推送）。发布镜像1445 passed/2 skipped、专项122 passed；API/优先Worker healthy。仅重试环评997：41265单元格、828245字正文、519实际子片段，21460行语义完整（1行尾空白被去除），三词检索定位通过。状态服务60/519向量、0失败；高级表头理解仍待完成，403副本及其他旧失败不动 |

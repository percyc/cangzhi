...
重导数据库表时，列名、顺序、推断类型、样例与统计始终重新计算。AI 字段解释采用独立
的成本策略：默认 `smart` 按“字段名 + 推断类型”复用旧说明，只把新增、类型变化或缺失
说明的字段交给语义任务；`full` 才请求全部字段。复用发生在新数据集版本事务内，不跨越
名称或类型变化，也不修改任何事实值。导入响应与数据集 profile 记录复用数和预计 AI
字段数，单次仍受 320 字段上限保护。

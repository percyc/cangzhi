...
doctor: ## Check containers, schema revision, and HTTP health endpoints
	...
	@docker compose exec -T web node -e "const raw = (process.env.CANGZHI_WEB_BASE_PATH || '').trim(); const p = raw && raw !== '/' ? '/' + raw.replace(/^\/+|\/+$$/g, '') : ''; require('http').get('http://localhost:3000' + p + '/api/health', r => { let b=''; r.on('data', c => b += c); r.on('end', () => { console.log(b); process.exit(r.statusCode === 200 ? 0 : 1) }) }).on('error', () => process.exit(1))"
